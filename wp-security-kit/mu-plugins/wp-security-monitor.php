...
function wpsk_monitor_cron_shape(){
	$r=array();$cron=_get_cron_array();foreach(is_array($cron)?$cron:array()as $hooks)foreach($hooks as $hook=>$events)foreach($events as $e){$s=(string)($e['schedule']??'');if($s==='')continue;$r[]=array($hook,$s,wpsk_monitor_hash($e['args']??array()));}sort($r);return $r;
}
...
function wpsk_monitor_scan(){
	...
	global $wpdb;$recent=$wpdb->get_results("SELECT ID,post_title,post_name FROM {$wpdb->posts} WHERE post_date_gmt>=DATE_SUB(UTC_TIMESTAMP(),INTERVAL 10 MINUTE) AND post_type='post' AND post_status IN ('publish','future','draft') ORDER BY ID DESC LIMIT 100",ARRAY_A);if(count($recent)>=5)wpsk_monitor_alert('post_burst',count($recent).' posts created within 10 minutes','critical',array('latest_id'=>(int)$recent[0]['ID']));
}
