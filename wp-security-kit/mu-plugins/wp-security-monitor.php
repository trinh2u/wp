...
function wpsk_monitor_scan(){
	$b=get_option(WPSK_MONITOR_BASELINE,array());$needs_migration=!is_array($b)||empty($b['version'])||(int)$b['version']<WPSK_MONITOR_VERSION;$needs_manifest=!is_array($b)||empty($b['php_code']['files']);$scan=$needs_migration||$needs_manifest||time()-(int)get_option(WPSK_MONITOR_CODE_RUN,0)>=6*HOUR_IN_SECONDS;$c=wpsk_monitor_snapshot($scan);if(!$scan&&isset($b['php_code']))$c['php_code']=$b['php_code'];if($scan)update_option(WPSK_MONITOR_CODE_RUN,time(),false);
	if($needs_migration||$needs_manifest){update_option(WPSK_MONITOR_BASELINE,$c,false);delete_option(WPSK_MONITOR_PENDING);wpsk_monitor_history('baseline_initialized','Security baseline initialized or migrated with complete PHP manifest','info');return;}
